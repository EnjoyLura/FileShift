...
import { processFileUpload, getFileInfo, getFileForDownload } from '../services/file.service.js';
...
router.post('/upload', upload.single('file'), async (req: Request, res: Response) => {
  ...
  try {
    const result = await processFileUpload(req.user!.userId, req.file, toolId);
...
router.get('/:id/download', async (req: Request, res: Response) => {
  ...
  try {
    const { filePath, originalName, mimeType } = await getFileForDownload(id, req.user!.userId);
